...
func mockClusterRegisterSteps() {
	...
	mockOperatorManager.EXPECT().GetOperatorsByType(models.OperatorTypeBuiltin).Return([]*models.MonitoredOperator{}).Times(1)
}
	...
	Context("Update", func() {
		...
		Context("Monitored Operators", func() {
			It("OLM register default value - only builtins", func() {
				...
				Expect(reflect.TypeOf(reply)).Should(Equal(reflect.TypeOf(installer.NewRegisterClusterCreated())))
				actual := reply.(*installer.RegisterClusterCreated)
				for _, operator := range actual.Payload.MonitoredOperators {
					Expect(operator.OperatorType).To(Equal(models.OperatorTypeBuiltin))
				}
			})
			...
			It("OLM register non default value", func() {
				...
				for _, operator := range actual.Payload.MonitoredOperators {
					if operator.OperatorType == models.OperatorTypeOlm {
						Expect(operator.Name).To(Equal(newOperatorName))
						Expect(operator.Properties).To(Equal(newProperties))
					}
				}
			})
